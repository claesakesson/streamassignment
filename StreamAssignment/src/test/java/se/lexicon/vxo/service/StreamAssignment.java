...
public class StreamAssignment {
    ...
    public void task9(){
        ...
        dtoList = people.stream().filter(person -> person
                .getDateOfBirth().isBefore(date))
                .map(person -> new PersonDto(person.getPersonId()
                        , person.getFirstName() + person.getLastName())).collect(Collectors.toList());
    ...
    public void task10(){
        ...
        optional = people.stream().filter(person -> person.getPersonId() == personId).findAny().map(person -> person.getDateOfBirth().format(DateTimeFormatter.ofPattern("eeee dd MMMM YYYY").withLocale(Locale.ENGLISH)).toUpperCase());
